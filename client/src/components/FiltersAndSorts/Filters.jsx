  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex flex-wrap">
        {reFil[0] &&
          reFil.map((e) => (
            <div className="inline-flex m-1 rounded-md bg-white" key={e.value}>
              <p className="justify-self-start mr-1">{e.value}</p>
              <button
                className="bg-white bg-slate-100 flex justify-self-end text-black text-[12px] hover:bg-white "
                value={e.value}
      ...
      {reFil[0] && reFil.find((e) => e.propety === "category") ? null : (
        <div className="my-3">
          <label className="font-bold block my-2  rounded-lg">Category</label>
          {reCategory.map((e) => (
            <div className="flex ">
              <p
                className="hover:cursor-pointer hover:ml-1 text-sm"
                onClick={handelrCate}
              >
                {e}
              </p>{" "}
      ...
      {reFil[0] && reFil.find((e) => e.propety === "model") ? null : (
        <div className="my-3">
          <label className="font-bold block my-2  rounded-lg">Model</label>
          {reModel.map((e) => (
            <div className="flex ">
              <p
                className="hover:cursor-pointer hover:ml-1 text-sm"
                onClick={handelrModel}
              >
                {e}
              </p>{" "}
              <br />
      ...
      {reFil[0] && reFil.find((e) => e.propety === "type") ? null : (
        <div className="my-3">
          <label className="font-bold block my-2  rounded-lg">Type</label>
          {reType.map((e) => (
            <div className="flex ">
              <p
                className="hover:cursor-pointer hover:ml-1 text-sm"
